...
const {
  ...
  FREEBUFF_CLI_USER_AGENT, CANONICAL_MODEL_ALIASES,
  logDebug, logInfo, logWarn, logError,
  debounceRequest, isBlacklistedModel, canonicalModelName,
  getApiUserAgent, getChatUserAgent, getAdsUserAgent,
  debugLog, httpGet, versionCompare, parseDuration,
} = require('./src/core');

const { createStateStore } = require('./src/config/state');
const { loadConfig: loadConfigFactory, saveConfig: saveConfigFactory } = require('./src/config/config');
...
const { normalizeChatMessages, normalizeAdMessages, normalizeMultimodalContent } = require('./src/requests/messages');
const { normalizeToolSchemas } = require('./src/requests/tools');
const { isNodeStream, readBodyText, pipeBodyToResponse, generateClientSessionId, cloneMap, cloneSlice, countOpenAIPayloadTokens } = require('./src/requests/utilities');
const { convertClaudeMessagesRequestToOpenAI, convertOpenAINonStreamResponseToClaude } = require('./src/requests/anthropic');
...
function logModelMismatch(requestedModel, actualModel, reason, tokenIdx) {
  const entry = {
    requested: requestedModel, actual: actualModel, reason: reason || 'unknown',
    token: tokenIdx != null ? `token-${tokenIdx + 1}` : null, at: new Date().toISOString(),
  };
...
function loadFreebuffCLITokens() {
  ...
  const searchPaths = [];
  const seen = new Set();
  const addPath = (p) => { const resolved = path.resolve(p); if (!seen.has(resolved)) { seen.add(resolved); searchPaths.push(resolved); } };

  const home = os.homedir();
  addPath(path.join(home, subPath));
  const envCandidates = [process.env.USERPROFILE, process.env.HOME, (process.env.HOMEDRIVE && process.env.HOMEPATH) ? path.join(process.env.HOMEDRIVE, process.env.HOMEPATH) : null, process.env.APPDATA, process.env.LOCALAPPDATA, process.env.XDG_CONFIG_HOME].filter(Boolean);
  for (const envDir of envCandidates) { addPath(path.join(envDir, subPath)); if (path.basename(envDir) !== 'manicode') addPath(path.join(envDir, credFile)); }
    ...
    try {
      ...
      if (fs.existsSync(usersDir)) {
        for (const entry of fs.readdirSync(usersDir)) {
          ...
          try { if (!fs.statSync(userDir).isDirectory()) continue; } catch (e) { continue; }
  ...
  for (const credPath of searchPaths) {
    if (fs.existsSync(credPath)) {
      ...
      try {
        ...
        for (const [key, value] of Object.entries(data)) { if (key !== 'default' && value && value.authToken) tokens.push(value.authToken); }
  ...
  for (const credPath of watchedPaths) {
    try {
      ...
      for (const entry of entries) {
        ...
        accounts.push({ token: entry.authToken, accountId: entry.accountId || entry.userId || user.id || null, email: entry.email || user.email || null, source: 'cli' });
      }
    } catch (_) {}
  }
...
function loadConfig() {
  ...
  return loadConfigFactory({ rootDir, loadCLITokens: loadFreebuffCLITokens, parseDuration, logInfo });
}
...
function loadState() { return stateStore.load(); }
function saveState(state) { return stateStore.save(state); }
...
async function checkAndUpdateVersions() {
  ...
  try {
    ...
    if (status === 200) {
      ...
      if (bunMatch && bunMatch[1] !== runtime.bunVersion) { updates.push(`Bun: ${runtime.bunVersion} -> ${bunMatch[1]}`); runtime.bunVersion = bunMatch[1]; }
    }
  } catch (e) { console.error(`[Versions] Failed to fetch RS source: ${e.message}`); }
  try {
    const { status: npmStatus, data: npmData } = await httpGet('https://registry.npmjs.org/freebuff/latest');
    if (npmStatus === 200) {
      try { const pkg = JSON.parse(npmData); if (pkg.version && pkg.version !== runtime.freebuffCliVersion) { updates.push(`Freebuff-CLI: ${runtime.freebuffCliVersion} -> ${pkg.version}`); runtime.freebuffCliVersion = pkg.version; runtime.aiSdkCompatVersion = pkg.version; } } catch (e) {}
    }
  } catch (e) { console.error(`[Versions] Failed to fetch npm registry: ${e.message}`); }
  if (updates.length > 0) { console.log(`[Versions] Updated: ${updates.join(', ')}`); return true; }
  return false;
}
...
function collectAllUserOpencodePaths() {
  ...
  if (process.platform === 'win32') {
    ...
    const userProfiles = [process.env.USERPROFILE, process.env.HOMEDRIVE ? path.join(process.env.HOMEDRIVE, process.env.HOMEPATH || '') : null].filter(Boolean);
    ...
    for (const ud of userDirs) { candidates.push(path.join(ud, '.config', 'opencode', 'opencode.json')); candidates.push(path.join(ud, '.opencode', 'opencode.json')); }
    if (fs.existsSync('C:\\Windows\\system32\\config\\systemprofile')) { candidates.push(path.join('C:\\Windows\\system32\\config\\systemprofile', '.config', 'opencode', 'opencode.json')); candidates.push(path.join('C:\\Windows\\system32\\config\\systemprofile', '.opencode', 'opencode.json')); }
    if (fs.existsSync('C:\\Windows\\ServiceProfiles\\LocalService')) { candidates.push(path.join('C:\\Windows\\ServiceProfiles\\LocalService', '.config', 'opencode', 'opencode.json')); candidates.push(path.join('C:\\Windows\\ServiceProfiles\\LocalService', '.opencode', 'opencode.json')); }
    if (fs.existsSync('C:\\Windows\\ServiceProfiles\\NetworkService')) { candidates.push(path.join('C:\\Windows\\ServiceProfiles\\NetworkService', '.config', 'opencode', 'opencode.json')); candidates.push(path.join('C:\\Windows\\ServiceProfiles\\NetworkService', '.opencode', 'opencode.json')); }
  } else {
    candidates.push(path.join(os.homedir(), '.config', 'opencode', 'opencode.json'));
    candidates.push(path.join(os.homedir(), '.opencode', 'opencode.json'));
...
function discoverOpencodeConfigsAsync() {
  if (cachedOpencodeConfigPaths !== null) return Promise.resolve([...cachedOpencodeConfigPaths]);
  ...
  return new Promise((resolve) => {
    try {
      ...
      const child = exec(command, { timeout: 15000, encoding: 'utf8', maxBuffer: 10 * 1024 * 1024 }, (err, stdout, stderr) => {
        const found = (stdout || '').trim().split('\n').filter(Boolean).map(s => s.trim()).filter(s => s.toLowerCase().endsWith('opencode.json'));
        if (found.length > 0) { logInfo(`[Opencode] Discovered ${found.length} config(s): ${found.join(', ')}`); cachedOpencodeConfigPaths = [...new Set([...existingFallbacks, ...found])].filter(p => fs.existsSync(path.dirname(p))); resolve([...cachedOpencodeConfigPaths]); return; }
    ...
    } catch (e) { logInfo(`[Opencode] Discovery failed (${e.message}), using fallback paths (${existingFallbacks.length})`); cachedOpencodeConfigPaths = [...existingFallbacks]; resolve([...cachedOpencodeConfigPaths]); }
  });
}
...
async function setupOpencodeConfig(skipRemovalSync) {
  ...
  if (unmatchedEnabled.length > 0) logWarn(`[Opencode] Warning: enabled models not found in registry: ${unmatchedEnabled.join(', ')}`);
  ...
  for (const configFile of configPaths) {
    try {
      ...
      if (fs.existsSync(configFile)) {
        ...
        if (!fs.existsSync(backupFile)) { fs.copyFileSync(configFile, backupFile); logInfo(`[Opencode] Backup created: ${backupFile}`); firstRun = true; }
      } else { logInfo(`[Opencode] No existing config found, will create: ${configFile}`); firstRun = true; }
      ...
      const existingModels = existing.provider['freebuff'] && existing.provider['freebuff'].models && Object.keys(existing.provider['freebuff'].models).length > 0
        ? Object.keys(existing.provider['freebuff'].models).map(canonicalModelName) : null;
      if (existingModels && existingModels.length > 0 && !skipRemovalSync) {
        ...
        const removedFromProvider = allRegistryModels.filter(m => enabledSet.has(canonicalModelName(m)) && !existingModels.includes(canonicalModelName(m)));
        if (removedFromProvider.length > 0) { config.enabledModels = config.enabledModels.filter(m => !removedFromProvider.includes(canonicalModelName(m))); saveConfig(config); for (const rm of removedFromProvider) logInfo(`[Opencode] Detected manual removal of ${rm}, removed from ENABLED_MODELS`); }
      }
      ...
      for (const m of allRegistryModels) {
        ...
        if (meta) { if (meta.modalities) entry.modalities = meta.modalities; const limit = modelRegistry.normalizeOpenCodeLimit(meta.limit); if (limit) entry.limit = limit; }
        models[m] = entry;
      }
      ...
      existing.provider['freebuff'] = { npm: '@ai-sdk/openai-compatible', name: 'Freebuff Proxy', options: { baseURL: `http://localhost:${parseInt(config.listenAddr.replace(':', '')) || 8080}/v1` }, models };
      fs.writeFileSync(configFile, JSON.stringify(existing, null, 2));
      logInfo(`[Opencode] Config updated: ${configFile}`);
    } catch (e) { logError(`[Opencode] Failed to update ${configFile}: ${e.message}`); }
  }
  return firstRun;
}
...
function buildAgentValidationPayload() {
  ...
  return {
    agentDefinitions: agents.map(a => ({
      id: a.id, publisher: 'codebuff', model: a.model, displayName: `Freebuff ${a.model}`,
      ...
      outputMode: 'last_message', includeMessageHistory: true,
      toolNames: a.spawnable.length > 0 ? ['spawn_agents'] : [], spawnableAgents: a.spawnable,
...
// ─── Gemini helpers ───
function isGeminiModel(canonicalModel) { return canonicalModel.startsWith('google/gemini-'); }
...
function isSessionInvalid(statusCode, errorBody) {
  ...
  try { const payload = JSON.parse(errorBody); const error = payload.error || payload.code || ''; return ['freebuff_update_required', 'waiting_room_required', 'waiting_room_queued', 'session_superseded', 'session_expired', 'session_model_mismatch'].includes(error); } catch (e) { return false; }
}
function isRunInvalid(statusCode, body) { if (statusCode !== 400) return false; const msg = body.toLowerCase(); return msg.includes('runid not found') || msg.includes('runid not running'); }

// ─── Country Detection ───
async function detectCountry() {
  try { const resp = await fetch('https://ipapi.co/json/', { signal: AbortSignal.timeout(5000) }); if (resp.ok) { const data = await resp.json(); if (data.country_code) { runtime.detectedCountry = data.country_code; console.log(`[Country] Detected: ${runtime.detectedCountry}`); return; } } } catch (_) {}
  try { const resp = await fetch('https://ipinfo.io/json', { signal: AbortSignal.timeout(5000) }); if (resp.ok) { const data = await resp.json(); if (data.country) { runtime.detectedCountry = data.country; console.log(`[Country] Detected: ${runtime.detectedCountry}`); return; } } } catch (_) {}
        ...
        tokenWatcherDebounce = setTimeout(async () => {
          ...
          if (added.length > 0) {
            ...
            for (const token of added) {
              await accountMgr.resolveAndReconcileToken(token, (loadFreebuffCLITokens().accounts || []).find(account => account.token === token) || {}, new UpstreamClient(config));
              const result = await tokenValidation.validateToken(token);
              if (result.valid) { config.authTokens.push(token); logInfo(`[Token Watch] Added token: ${token.substring(0, 8)}...`); }
              else logWarn(`[Token Watch] Skipped invalid token: ${token.substring(0, 8)}... (${result.status})`);
            }
          }
          if (removed.length > 0) {
          ...
          if (added.length > 0 || removed.length > 0) { saveConfig(config); await tokenValidation.reloadTokenPool(); logInfo(`[Token Watch] TokenPool reloaded: ${config.authTokens.length} token(s)`); }
        }, 1000);
      });
      watched.add(credPath);
      logDebug(`[Token Watch] Watching ${credPath}`);
    } catch (e) { logWarn(`[Token Watch] Could not watch ${credPath}: ${e.message}`); }
  }
}
...
const ModelRegistryClass = createModelRegistry({
  ...
  sourceUrls: { models: FREEBUFF_MODELS_SOURCE_URL, agents: FREE_AGENTS_SOURCE_URL, ids: FREEBUFF_MODEL_IDS_SOURCE_URL, config: MODEL_CONFIG_SOURCE_URL },
  isBlacklistedModel, canonicalModelName, logInfo, logError,
});

const UpstreamClientClass = createUpstreamClient({
  CODEBUFF_ACCEPT_ENCODING, CODEBUFF_JSON_USER_AGENT, FREEBUFF_CLI_USER_AGENT,
  getChatUserAgent, logDebug, logInfo, logWarn, logError, debugLog,
  buildAgentValidationPayload, normalizeAdMessages, nodeFetch,
...
const runChains = createRunChains({
  ...
  getGeminiSubagentId, isGeminiModel, logError,
});
...
const TokenPoolClass = createTokenPool({
  ...
  quotaSummary: (quota) => {
    ...
    if (models.length > 0) {
      const selected = models.reduce((best, value) => { const remaining = Math.max(0, Number(value.limit) - Number(value.recentCount || 0)); return !best || remaining > best.remaining ? { value, remaining } : best; }, null);
      return { used: Number(selected.value.recentCount) || 0, limit: Number(selected.value.limit) || 0, resetAt: selected.value.resetAt || quota.resetAt || null };
    }
    return { used: Number(quota?.recentCount) || 0, limit: Number(quota?.limit) || 0, resetAt: quota?.resetAt || null };
  },
  logDebug, logInfo, logWarn, logError, pushEvent,
});

const accountMgr = createAccountManager({
  crypto, getConfig: () => config, getTokenPool: () => tokenPool, saveConfig, logInfo, logDebug, logWarn, logError,
});
...
const responseWriters = createResponseWriters({ isNodeStream, readBodyText, convertOpenAINonStreamResponseToClaude });

const proxyChatRequestFn = createProxyChatRequest({
  canonicalModelName, FALLBACK_AGENT_IDS, GEMINI_PARENT_AGENT_ID, crypto,
  markAccountUsed: accountMgr.markAccountUsed,
  logModelMismatch, logInfo, logWarn, logError, logDebug, debugLog, pushEvent,
  isGeminiModel, getGeminiSubagentId,
  startRunChainGemini: runChains.startGemini, startRunChainNormal: runChains.startNormal,
  finalizeRunChainGemini: runChains.finalizeGemini, finalizeRunChainNormal: runChains.finalizeNormal,
  normalizeChatMessages, cloneMap, normalizeToolSchemas, generateClientSessionId,
  readBodyText, isSessionInvalid, isRunInvalid,
  getTokenPool: () => tokenPool,
...
const httpHandlers = createHttpHandlers({
  ...
  getTokenPool: () => tokenPool,
  getModelRegistry: () => modelRegistry,
  startTime, runtime, MODEL_MISMATCH_LOG, EVENT_LOG,
  http, fs, path, url, crypto, debounceRequest,
  proxyChatRequest: proxyChatRequestFn,
  setupOpencodeConfig, saveConfig,
  ...
  FREEBUFF_MODELS_SOURCE_URL, FREEBUFF_MODEL_IDS_SOURCE_URL, FREE_AGENTS_SOURCE_URL, MODEL_CONFIG_SOURCE_URL, MODEL_REFRESH_INTERVAL,
  logDebug, logInfo, logWarn, logError,
  ...
  errorWriters, requestUtilities: { isNodeStream, readBodyText, pipeBodyToResponse, countOpenAIPayloadTokens },
  responseWriters, anthropicRequests: { convertClaudeMessagesRequestToOpenAI, convertOpenAINonStreamResponseToClaude },
  IS_BUN, RUNTIME_VERSION,
});
...
async function startServer() {
  ...
  await detectCountry();
  if (config.mockCountry) { runtime.detectedCountry = config.mockCountry; logInfo(`[Country] MOCKED to: ${runtime.detectedCountry}`); }
  ...
  server.listen(port+1, '0.0.0.0', () => {
    ...
    if (firstRun) {
      ...
      if (process.platform === 'win32') require('child_process').exec(`start "" "${dashboardUrl}"`);
      else if (process.platform === 'darwin') require('child_process').exec(`open "${dashboardUrl}"`);
  ...
  setInterval(async () => {
    ...
    if (newTokens.length > 0) {
      ...
      for (const token of newTokens) {
        await accountMgr.resolveAndReconcileToken(token, cliAccounts.find(account => account.token === token) || {}, new UpstreamClientClass(config));
        const result = await tokenValidation.validateToken(token);
        if (result.valid) { config.authTokens.push(token); logInfo(`Added valid token: ${token.substring(0, 8)}...`); changed = true; }
        else logWarn(`Skipped invalid new token: ${token.substring(0, 8)}... (${result.status})`);
      }
    }
    ...
    if (changed) { saveConfig(config); await tokenValidation.reloadTokenPool(); }
  ...
  setInterval(async () => {
    ...
    for (const result of results) {
      ...
      tokenPool.setTokenHealth(result.token, result);
      if (previous.status !== result.status) { if (result.valid) logInfo(`Token ${result.token.substring(0, 8)}... became active`); else logWarn(`Token ${result.token.substring(0, 8)}... became ${result.status}`); }
      if (!result.valid && (result.status === 'banned' || result.status === 'unauthorized')) await tokenPool.endAllSessionsForToken(result.token);
    }
  ...
  setInterval(async () => {
    try { await accountMgr.checkIdleAccounts(); } catch (e) { logWarn(`[Account] Idle check failed: ${e.message}`); }
  }, 12 * 1000);

  setInterval(async () => {
    try { await checkAndUpdateVersions(); } catch (e) {}
    try { await checkProxyVersion(); } catch (e) {}
  }, 60 * 60 * 1000);
}
