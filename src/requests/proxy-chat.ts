function createProxyChatRequest(deps) {
  ...
  const tokenPool = new Proxy(
    ...
    {
      get(_, property) {
        const value = deps.getTokenPool()?.[property];
        return typeof value === "function"
          ? value.bind(deps.getTokenPool())
          : value;
      },
    },
  );
  const modelRegistry = new Proxy(
    ...
    {
      get(_, property) {
        const value = deps.getModelRegistry()?.[property];
        return typeof value === "function"
          ? value.bind(deps.getModelRegistry())
          : value;
      },
    },
  );
  async function proxyChatRequest(
    ...
    try {
      await client.validateAgents(token);
    } catch (_) {}
    try {
      await client.requestAds(token, "gravity", payload.messages || []);
    } catch (_) {}
    try {
      await client.getStreak(token);
    } catch (_) {}
    ...
    for (let attempt = 0; attempt < 10; attempt++) {
      ...
      let accessTier = null;
      try {
        const session = await tokenPool.ensureSession(
          currentToken,
          currentModel,
        );
        ...
        const isRateLimited =
          e.message &&
          (e.message.includes("429") || e.message.includes("rate_limited"));
        if (isRateLimited) {
          ...
          if (tokenPool.tokens.length > preReloadTried) {
            logInfo(
              `[Token Rotate] Pool has ${tokenPool.tokens.length} tokens but only ${preReloadTried} tried, checking for new tokens...`,
            );
          }
          const nextToken = tokenPool.getToken(currentModel, triedTokens);
          if (nextToken && !triedTokens.has(nextToken)) {
            logInfo(
              `[Token Rotate] ${currentToken.substring(0, 8)}... rate limited on ${currentModel}, trying ${nextToken.substring(0, 8)}...`,
            );
          ...
          if (tokenPool.tokens.length > triedTokens.size) {
            const hotReloadToken = tokenPool.getToken(
              currentModel,
              triedTokens,
            );
            if (hotReloadToken) {
              logInfo(
                `[Token Rotate] Found new token ${hotReloadToken.substring(0, 8)}... from hot-reload, trying it`,
              );
        ...
        writeError(
          res,
          502,
          `failed to acquire upstream free session: ${e.message}`,
          "server_error",
          "",
        );
        return;
      ...
      if (actualModel !== requestedModel) {
        logModelMismatch(
          requestedModel,
          actualModel,
          "session_created_with_different_model",
          tokenPool.tokens.indexOf(currentToken),
        );
      }

      const canonicalModel = canonicalModelName(actualModel);
      const agentID =
        modelRegistry.getAgentForModel(canonicalModel) ||
        FALLBACK_AGENT_IDS[canonicalModel] ||
        "base2-free";
      ...
      try {
        ...
        writeError(
          res,
          502,
          `failed to start run chain: ${e.message}`,
          "server_error",
          "",
        );
        return;
      ...
      const requestedDisplay =
        actualModel !== requestedModel
          ? ` (locked from ${requestedModel})`
          : "";
      const chatRunId = isGemini ? run.chatRunId : run.runId;
      logInfo(
        `[Request] model: ${actualModel}${requestedDisplay}, run: ${run.runId}${isGemini ? ` (child: ${chatRunId})` : ""}, tier: ${accessTier || "normal"}`,
      );
      ...
      if (
        cloned.reasoning_effort !== undefined ||
        cloned.reasoningEffort !== undefined ||
        (cloned.reasoning && cloned.reasoning.effort !== undefined)
      ) {
        logInfo(
          `[Normalize] reasoning_effort=${JSON.stringify(cloned.reasoning_effort)}, reasoningEffort=${JSON.stringify(cloned.reasoningEffort)}, reasoning.effort=${JSON.stringify(cloned.reasoning && cloned.reasoning.effort)}`,
        );
      }
      ...
      const effortValue =
        ...
        (cloned.reasoning && cloned.reasoning.effort);
      ...
      if (actualModel && actualModel.includes("luna")) {
        debugLog({
          ...
          payload: JSON.parse(JSON.stringify(cloned)),
        });
      }

      let resp;
      try {
        resp = await client.chatCompletions(currentToken, cloned);
      } catch (e) {
        if (actualModel && actualModel.includes("luna"))
          debugLog({
      ...
      if (actualModel && actualModel.includes("luna") && resp.status >= 400) {
        ...
        debugLog({
          ...
          model: actualModel,
          status: resp.status,
          body: errBody.substring(0, 2000),
        });
        // Reconstruct a new response since we consumed the body
        resp = {
          status: resp.status,
          body: require("stream").Readable.from(Buffer.from(errBody)),
      ...
      if (resp.status === 429) {
        ...
        pushEvent("warn", "Rate limited (429)", {
          detail: errorBodyStr.substring(0, 200),
          model: actualModel,
        });
        for (let retry = 0; retry < 3; retry++) {
          const waitMs = (retry + 1) * 3000;
          logInfo(
            `[Rate Limit] Waiting ${waitMs / 1000}s before retry ${retry + 1}/3...`,
          );
      ...
      if (resp.status >= 200 && resp.status < 300) {
        ...
        try {
          const result = await writeSuccess(res, resp);
          messageId = result.messageId;
          actualResponseModel = result.model;
        } catch (e) {
          logError(`proxy response copy failed: ${e.message}`);
        }
        logInfo(
          `[Response] model: ${actualResponseModel || actualModel}, completed in ${Date.now() - reqStart}ms (status: ${resp.status})`,
        );
        setImmediate(() =>
          isGemini
            ? finalizeRunChainGemini(client, currentToken, run, messageId)
            : finalizeRunChainNormal(client, currentToken, run, messageId),
        );
        return;
      }
      ...
      const errorBodyStr = await readBodyText(resp.body);
      logError(
        `[Upstream Error] ${resp.status}: ${errorBodyStr.substring(0, 200)}`,
      );
      pushEvent("error", `[Upstream Error] ${resp.status}`, {
        detail: errorBodyStr.substring(0, 200),
        model: actualModel,
        status: resp.status,
      });
      ...
      if (isSessionInvalid(resp.status, errorBodyStr)) {
          ...
          if (errorType === "session_model_mismatch") {
            ...
            if (!lockedModel && errorData.message) {
              const match = errorData.message.match(
                /bound to ([a-zA-Z0-9][a-zA-Z0-9._/-]+)/,
              );
              if (match)
                lockedModel = match[1].replace(/;.*$/, "").replace(/\.$/, "");
            }
            if (!lockedModel) {
            ...
            if (!lockedModel) {
              try {
                ...
                if (parsed.body && parsed.body.currentModel)
        ...
        logWarn(
          `[Session Invalid] status=${resp.status}, error=${errorType}${lockedModel ? ", lockedModel=" + lockedModel : ""}`,
        );
        pushEvent("warn", `Session invalid: ${errorType}`, {
          detail: `status=${resp.status}${lockedModel ? ", lockedModel=" + lockedModel : ""}`,
          model: actualModel,
          requestedModel,
          errorType,
        });
        ...
        if (errorType === "freebuff_update_required" || resp.status === 426) {
          logWarn(
            `[Version] Server requires update, invalidating session and retrying...`,
          );
          pushEvent("warn", "Server requires update", {
            detail: "freebuff_update_required",
            model: actualModel,
          });
        }
        tokenPool.invalidateSession(currentToken, actualModel);
        if (requestedModel !== actualModel)
          tokenPool.invalidateSession(currentToken, requestedModel);

        if (
          errorType === "session_model_mismatch" &&
          lockedModel &&
          lockedModel !== requestedModel &&
          !mismatchUnlockAttempted
        ) {
          mismatchUnlockAttempted = true;
          logModelMismatch(
            requestedModel,
            lockedModel,
            "session_model_mismatch_unlock_retry",
            tokenPool.tokens.indexOf(currentToken),
          );
          logInfo(
            `[Model Lock] Mismatch: session bound to ${lockedModel}. Ending session to unlock and retrying requested model ${requestedModel}`,
          );
          pushEvent(
            "model_switch",
            `Unlocking session to retry requested model`,
            {
              from: lockedModel,
              to: requestedModel,
              reason: "session_model_mismatch_unlock_retry",
              token: currentToken.substring(0, 8) + "...",
            },
          );
        ...
        if (lockedModel) {
          logModelMismatch(
            currentModel,
            lockedModel,
            "model_locked_fallback",
            tokenPool.tokens.indexOf(currentToken),
          );
          logInfo(
            `[Model Lock] Switching from ${currentModel} to ${lockedModel}`,
          );
          pushEvent("model_switch", `Switched to locked model`, {
            from: currentModel,
            to: lockedModel,
            reason: "model_locked_fallback",
            token: currentToken.substring(0, 8) + "...",
          });
    ...
    if (lastRateLimitError) {
      ...
      for (const candidate of fallbackCandidates) {
        try {
          logInfo(
            `[Fallback] All tokens rate limited on ${requestedModel}, trying model ${candidate}`,
          );
          const session = await tokenPool.ensureSession(
            fallbackToken,
            candidate,
          ...
          logModelMismatch(
            ...
            candidate,
            "rate_limit_fallback",
            tokenPool.tokens.indexOf(fallbackToken),
          );
          ...
          try {
            ...
            logWarn(
              `[Fallback] run chain failed for ${candidate}: ${e.message}`,
            );
            continue;
          }
          const normalizedMessages = normalizeChatMessages(payload.messages);
          ...
          try {
            ...
            logWarn(
              `[Fallback] chat request failed for ${candidate}: ${e.message}`,
            );
            continue;
          }
          if (resp.status >= 200 && resp.status < 300) {
            ...
            logInfo(
              `[Response] model: ${actualResponseModel || candidate}, completed in ${Date.now() - reqStart}ms (status: ${resp.status}) [fallback]`,
            );
            setImmediate(() =>
              isGemini
                ? finalizeRunChainGemini(client, currentToken, run, messageId)
      ...
      logModelMismatch(
        ...
        tokenPool.tokens.indexOf(token),
      );
    ...
    writeError(
      res,
      502,
      "upstream run expired twice in a row",
      "server_error",
      "",
    );
  }
