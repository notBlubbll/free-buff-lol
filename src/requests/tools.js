...
function normalizeSchemaMap(node, defs, maxDepth) {
  ...
  if (replaced && typeof replaced === 'object' && !Array.isArray(replaced)) return normalizeSchemaMap(replaced, defs, maxDepth - 1);
  const normalized = {};
...
function normalizeSchemaValue(value, defs, maxDepth) {
  ...
  if (Array.isArray(value)) return value.map(item => normalizeSchemaValue(item, defs, maxDepth));
  return value;
}
...
function tryResolveRef(node, defs) {
  ...
  const name = node.$ref.startsWith(prefix) ? node.$ref.slice(prefix.length) : '';
  if (!name || !defs[name]) return null;
  const definition = defs[name];
  return typeof definition === 'object' && !Array.isArray(definition) ? cloneMap(definition) : definition;
}
...
function normalizeTypeField(schema) {
  ...
  const nonNull = schema.type.filter(type => typeof type === 'string' && type !== 'null' && type.trim());
