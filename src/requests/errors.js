function createErrorWriters({ http }) {
  ...
  function writeClaudeError(res, statusCode, message, errorType) {
    if (!message) message = http.STATUS_CODES[statusCode] || 'Unknown error';
    writeJSON(res, statusCode, { type: 'error', error: { type: errorType || 'api_error', message } });
  }
  ...
  function writePassthroughError(res, statusCode, body) {
    ...
    try {
      ...
      writeOpenAIError(res, statusCode, payload.error?.message || payload.message || trimmed, payload.error?.type || 'upstream_error', payload.error?.code || '');
    } catch (_) {
  ...
  function writeClaudePassthroughError(res, statusCode, body) {
    ...
    try {
      const payload = JSON.parse(trimmed);
      writeClaudeError(res, statusCode, payload.error?.message || payload.message || trimmed, 'api_error');
    } catch (_) {
      writeClaudeError(res, statusCode, trimmed, 'api_error');
    }
  }

  return { writeJSON, writeOpenAIError, writeClaudeError, writePassthroughError, writeClaudePassthroughError };
}
