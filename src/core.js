...
function logAt(level, ...args) {
  if ((LOG_LEVELS[level] || 0) <= (LOG_LEVELS[LOG_LEVEL] || 2)) console.log(`[${level.toUpperCase()}]`, ...args);
}
...
function isBlacklistedModel(modelId) {
  return !!modelId && typeof modelId === 'string' && BLACKLISTED_MODEL_PATTERNS.some(re => re.test(modelId));
}
function canonicalModelName(model) { return CANONICAL_MODEL_ALIASES[model] || model; }
function getApiUserAgent() { return `Bun/${runtime.bunVersion}`; }
function getChatUserAgent() { return `ai-sdk/openai-compatible/0.0.0-test/codebuff ai-sdk/provider-utils/${runtime.aiSdkProviderUtilsVersion} runtime/browser`; }
function getAdsUserAgent() { return `Freebuff-CLI/${runtime.freebuffCliVersion}`; }
...
function debugLog(entry) {
  try {
    ...
    fs.appendFileSync(debugLogPath, JSON.stringify({ ts: new Date().toISOString(), ...entry }, null, 2) + '\n---\n');
  } catch (_) {}
}

async function httpGet(requestUrl, options = {}) {
  return new Promise(resolve => {
    const req = https.get(requestUrl, { headers: { Accept: 'application/json', ...options.headers }, timeout: 10000 }, res => {
      let data = '';
...
function parseDuration(value) {
  ...
  return amount * ({ ms: 1, s: 1000, m: 60000, h: 3600000, d: 86400000 }[match[2]?.toLowerCase() || 'ms']);
}
...
module.exports = {
  ...
  CANONICAL_MODEL_ALIASES, FALLBACK_AGENT_IDS, GEMINI_PARENT_AGENT_ID,
  GEMINI_SUBAGENT_IDS, CONTEXT_PRUNER_AGENT_ID, CODEBUFF_ACCEPT_ENCODING,
  CODEBUFF_JSON_USER_AGENT, FREEBUFF_CLI_USER_AGENT, logDebug, logInfo,
  logWarn, logError, debounceRequest, isBlacklistedModel, canonicalModelName,
  getApiUserAgent, getChatUserAgent, getAdsUserAgent, debugLog, httpGet,
  versionCompare, parseDuration
};
