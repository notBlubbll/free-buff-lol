function createUpstreamClient(deps) {
  ...
  class UpstreamClient {
    ...
    async startRun(authToken, agentID, ancestorRunIds = []) {
      const resp = await this.doJSON(authToken, "/api/v1/agent-runs", {
        action: "START",
        agentId: agentID,
        ancestorRunIds,
      });
      if (resp.status < 200 || resp.status >= 300)
        throw new Error(`start run failed ${resp.status}: ${resp.body}`);
      const parsed = JSON.parse(resp.body);
      if (!parsed.runId)
        throw new Error(`start run response missing runId: ${resp.body}`);
      return parsed.runId;
    }
    ...
    async finishRun(authToken, runID, totalSteps) {
      ...
      if (resp.status < 200 || resp.status >= 300)
        throw new Error(`finish run failed ${resp.status}: ${resp.body}`);
    }
    ...
    async recordRunStep(
      ...
      if (resp.status < 200 || resp.status >= 300)
        throw new Error(`record run step failed ${resp.status}: ${resp.body}`);
    }
    ...
    chatCompletions(authToken, body) {
      ...
      return nodeFetch(requestURL, fetchOpts)
        .then((resp) => {
          clearTimeout(timer);
          const responseHeaders = {};
          resp.headers.forEach((v, k) => (responseHeaders[k] = v));
    ...
    async doSessionRequest(
      ...
      try {
        ...
        const data = await resp.text();
        logDebug(
          `Session ${method} response (${resp.status}): ${data.substring(0, 300)}`,
        );
        if (resp.status === 404) return { status: "disabled" };
        if (resp.status < 200 || resp.status >= 300) {
          if (resp.status === 426 || data.includes("freebuff_update_required"))
            throw new Error("freebuff_update_required");
          if (data.includes("model_locked"))
            throw new Error(
              JSON.stringify({ type: "model_locked", body: JSON.parse(data) }),
            );
          throw new Error(
            `free session request failed ${resp.status}: ${data}`,
          );
        }
        try {
          return JSON.parse(data);
        } catch (e) {
          throw new Error("decode session: " + e.message);
        }
      } catch (e) {
        clearTimeout(timer);
        throw e;
    ...
    async validateAgents(authToken) {
      ...
      if (resp.status >= 200 && resp.status < 300) {
        ...
        logWarn(
          `[Agents] Validation failed (${resp.status}), continuing with server configs`,
        );
      }
    }
    ...
    async requestAds(authToken, provider, messages = [], sessionId = "") {
      ...
      return await this.doJSON(authToken, "/api/v1/ads", body, "POST", {
        "User-Agent": FREEBUFF_CLI_USER_AGENT,
      });
    }

    async getStreak(authToken) {
      return await this.doJSON(
        authToken,
    ...
    async getAccountIdentity(authToken) {
      for (const pth of [
        ...
        if (response.status === 401 || response.status === 403)
          return {
            status: "unauthorized",
            error: `identity lookup failed ${response.status}`,
          };
        ...
        const user =
          body.user || body.account || body.profile || body.data || body;
        const accountId =
          user.id ||
          user.userId ||
          user.accountId ||
      ...
      return {
        status: "temporary",
        error: "upstream returned no account identity",
      };
    }
    ...
    async reportZeroclickImpression(authToken, ids) {
      ...
      try {
        ...
        const data = await resp.text();
        if (resp.status >= 400)
          logWarn(`[Ads] Zeroclick impression failed: ${resp.status}`);
        return { status: resp.status, body: data };
      } catch (e) {
        clearTimeout(timer);
        logError(`[Ads] Zeroclick error: ${e.message}`);
      }
    }
