function createRunChains({ contextPrunerAgentId, geminiParentAgentId, getGeminiSubagentId, isGeminiModel, logError }) {
  async function startNormal(client, token, agentId) {
    ...
    const childRunId = await client.startRun(token, contextPrunerAgentId, [runId]);
  ...
  async function finalizeNormal(client, token, run, messageId) {
    ...
    } catch (error) { logError(`finalize run failed: ${error.message}`); }
  }
  ...
  async function finalizeGemini(client, token, run, messageId) {
    ...
    } catch (error) { logError(`finalize gemini run failed: ${error.message}`); }
  }
  ...
  async function finalizeSimple(client, token, run, messageId) {
    try {
      ...
      await client.finishRun(token, run.runId, 2);
    } catch (error) { logError(`finalize simple run failed: ${error.message}`); }
  }

  return { startNormal, startGemini, finalizeNormal, finalizeGemini, startSimple, finalizeSimple };
}
