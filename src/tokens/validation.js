function createTokenValidation(deps) {
  const { getConfig, setConfig, getTokenPool, setTokenPool, loadConfig, TokenPool, UpstreamClient, logError, logWarn, logInfo, extractQuota } = deps;
...
async function probeNewModels(models) {
  ...
  return { validateToken, classifyTokenError, validateAllTokens, reloadTokenPool, probeNewModels };
}
