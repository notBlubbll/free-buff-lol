function createHttpHandlers(deps) {
  const { getConfig, getTokenPool, getModelRegistry, startTime, runtime, MODEL_MISMATCH_LOG, EVENT_LOG, http, fs, path, url, crypto, debounceRequest, proxyChatRequest, setupOpencodeConfig, saveConfig, reloadTokenPool, probeNewModels, resolveAndReconcileToken, UpstreamClient, getAdsUserAgent, loadFreebuffCLITokens, FREEBUFF_MODELS_SOURCE_URL, FREEBUFF_MODEL_IDS_SOURCE_URL, FREE_AGENTS_SOURCE_URL, MODEL_CONFIG_SOURCE_URL, MODEL_REFRESH_INTERVAL, logDebug, logInfo, logWarn, logError, accountForToken, accountCheckInfo, mergeQuotaSources, errorWriters, requestUtilities, responseWriters, anthropicRequests, IS_BUN, RUNTIME_VERSION } = deps;
  const config = new Proxy({}, { get: (_, key) => getConfig()?.[key], set: (_, key, value) => { getConfig()[key] = value; return true; } });
  const tokenPool = new Proxy({}, { get: (_, key) => { const value = getTokenPool()?.[key]; return typeof value === 'function' ? value.bind(getTokenPool()) : value; } });
  const modelRegistry = new Proxy({}, { get: (_, key) => { const value = getModelRegistry()?.[key]; return typeof value === 'function' ? value.bind(getModelRegistry()) : value; } });
  // --- HTTP Handlers ---
...
function writeOpenAIError(res, statusCode, message, errorType, code) {
  return errorWriters.writeOpenAIError(res, statusCode, message, errorType, code);
}
...
async function writeClaudeSuccessResponse(res, resp, requestedModel, stream) {
  return responseWriters.writeClaudeSuccessResponse(res, resp, requestedModel, stream);
}
...
async function handleRequest(req, res) {
  ...
  if (pathname === '/dashboard' || pathname === '/') {
    const dashboardPath = path.join(path.resolve(__dirname, '../..'), 'dashboard.html');
  ...
  if (pathname === '/api/models' && req.method === 'GET') {
    ...
    writeJSON(res, 200, {
      ...
      registry: {
        ...
        sources: { catalog: FREEBUFF_MODELS_SOURCE_URL, stable_ids: FREEBUFF_MODEL_IDS_SOURCE_URL, agent_mapping: FREE_AGENTS_SOURCE_URL, model_constants: MODEL_CONFIG_SOURCE_URL },
  ...
  if (pathname === '/api/usage' && req.method === 'GET') {
    ...
    if (!tp) { writeJSON(res, 503, { ok: false, error: 'token pool not ready' }); return; }
  ...
  if (pathname === '/api/session/unlock' && req.method === 'POST') {
    ...
    } catch (e) { writeJSON(res, 500, { ok: false, error: e.message }); }
    return;
  }
  ...
  return { handleRequest, handleHealthz, handleModels, handleChatCompletions, handleClaudeMessages, handleClaudeCountTokens };
}
