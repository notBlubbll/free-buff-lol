...
function loadConfig({ rootDir, loadCLITokens, parseDuration, logInfo }) {
  ...
  if (fs.existsSync(configPath)) {
    try { rawConfig = { ...rawConfig, ...JSON.parse(fs.readFileSync(configPath, 'utf8')) }; }
    catch (error) { console.error('Failed to parse config.json:', error.message); }
  }
  const env = process.env;
  if (env.LISTEN_ADDR) rawConfig.LISTEN_ADDR = env.LISTEN_ADDR;
  if (env.UPSTREAM_BASE_URL) rawConfig.UPSTREAM_BASE_URL = env.UPSTREAM_BASE_URL;
  if (env.REQUEST_TIMEOUT) rawConfig.REQUEST_TIMEOUT = env.REQUEST_TIMEOUT;
  if (env.AUTH_TOKENS) rawConfig.AUTH_TOKENS = env.AUTH_TOKENS.split(',').map(t => t.trim()).filter(Boolean);
  if (env.API_KEYS) rawConfig.API_KEYS = env.API_KEYS.split(',').map(t => t.trim()).filter(Boolean);
  if (env.ENABLED_MODELS) rawConfig.ENABLED_MODELS = env.ENABLED_MODELS.split(',').map(t => t.trim()).filter(Boolean);
  if (env.MOCK_COUNTRY) rawConfig.MOCK_COUNTRY = env.MOCK_COUNTRY.trim().toUpperCase();
  if (env.LOG_LEVEL) rawConfig.LOG_LEVEL = env.LOG_LEVEL;
  if (env.TOKEN_REVALIDATE_INTERVAL) rawConfig.TOKEN_REVALIDATE_INTERVAL = env.TOKEN_REVALIDATE_INTERVAL;
...
function saveConfig(rootDir, cfg) {
  ...
  if (!fs.existsSync(backupPath) && fs.existsSync(configPath)) {
    try { fs.copyFileSync(configPath, backupPath); } catch (error) { console.error('Failed to create config backup:', error.message); }
  }
